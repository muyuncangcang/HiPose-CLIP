...
from modules.prompts import visual_prompt, text_prompt  # 提示生成模块
...
from utils.attributes import k_means  # K-Means 聚类方法
...
def main(config: DictConfig):
    ...
    copy("/root/ddddddd/autolabel-main/configs/config.yaml", working_dir)
    copy("/root/ddddddd/autolabel-main/train.py", working_dir)
    ...
    fusion_model = visual_prompt(config.network.sim_header, clip_state_dict, config.data.num_segments)  # 融合模型
    # fusion_model = St_gcn(config.network.sim_header, clip_state_dict, config.data.num_segments)
    text_model = TextCLIP(model)  # 文本模型
    
    # cnn_model = CNNModule(input_dim=512, output_dim=156)
    # cnn_model = cnn_model.to(device)

    # CNN = CNNDLGA(input_size=512)
    # CNN = CNN.to(device)
